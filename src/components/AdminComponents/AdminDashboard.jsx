...
class AdminDashboard extends Component {
  ...
  getCohortAssignments = (assignmentArr, cohortId) => {
    let arr = assignmentArr.filter(
      assignment => assignment.cohort_id === cohortId
    );
    return arr;
  };
  ...
  TriggerAssignmentDetails = assignment => {
    this.setState({ showAssignmentDetails: assignment });
  };
  TriggerEventDetails = event => {
    this.setState({ showEventDetails: event });
  };
  TriggerEditCohortForm = cohort => {
    this.setState({ showEditCohortForm: cohort });
  };
  TriggerStudentDetails = studentId => {
    this.setState({
      showStudentDetails: this.state.students.filter(
      ...
      return (
        <AdminStudentDetails
          ...
          assignments={this.state.assignments.filter(
            assignment =>
              assignment.cohort_id === this.state.showStudentDetails.cohort_id
          )}
  ...
  escapeNewEventModal = () => {
    this.setState({ showNewEventForm: false });
  };
  escapeNewStudentModal = () => {
    this.setState({ showNewStudentForm: false });
  };
  escapeNewAssignmentModal = () => {
    this.setState({ showNewAssignmentForm: false });
  };
  escapeAssignmentDetailsModal = () => {
    this.setState({ showAssignmentDetails: false });
  };
  escapeNewCohortModal = () => {
    this.setState({ showNewCohortForm: false });
  };
  escapeNewAdminModal = () => {
    this.setState({ showNewAdminForm: false });
  };
  escapeEventDetailsModal = () => {
    this.setState({ showEventDetails: false });
  };
  escapeEditCohortModal = () => {
    this.setState({ showEditCohortForm: false });
  };
  escapeStudentDetails = () => {
  ...
  render() {
    ...
    if (
      ...
      typeof this.state.onFocusData != undefined
    ) {
      CohortDetail = (
        <CohortDetails
          ...
          submissions={this.state.submissions}
          cohortStudents={this.getCohortStudents(
            this.state.students,
            this.state.onFocusData.id
          )}
          cohortEvents={this.getCohortEvents(
            this.state.events,
            this.state.onFocusData.id
          )}
          cohortAssignments={this.getCohortAssignments(
