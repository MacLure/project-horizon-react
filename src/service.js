...
export const createNewAssignment = (data, token) => {
  return new Promise((resolve, reject) => {
    console.log(':::new Assignment::',`${rootURL}/admin/assignments?assignment=${data}`);
...
export const deleteAssignment = (assignment_id, token) => {
  return new Promise((resolve, reject) => {
    fetch(`${rootURL}/admin/assignments/${assignment_id}?${JSON.stringify({'assignment_id': assignment_id})}`, {
      method: 'delete',
      headers: {
        "Authorization": `${token}`,
        'Content-Type': 'application/json'
